$(async () => {
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    return `<p id="status-${row.id}">${data}</p>` +
                        ...
                        Selecter+
                        '</select>' +
                        `<button onclick="updateStatus(${row.id})" type="button" class="btn btn-primary my-2">Update</button>`
                }
            },
            {
                data: null,
                render: function (data, type, row) {
                    return '<button onclick="setTableModal(' + row.id + ')" type="button" class="btn btn-primary my-2" data-bs-toggle="modal" data-bs-target="#TableModal">Assign Tables</button>'
                        + '<button onclick="setEditModal(' + row.id + ')"  type="button" class="btn btn-primary m-2" data-bs-toggle="modal" data-bs-target="#EditModal">Edit</button>'
    ...
    $('#BackBtn').click(()=>{
        window.location.href = "/Management/Calendar/Index";
    });
